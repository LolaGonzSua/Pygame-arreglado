...
class Video:
    def __init__(self, path):
        ...
        self.active = True
        self.frame_surf = pygame.Surface(self.original_size)  # Inicializamos con el tamaño correcto
    ...
    def restart(self):
        ...
        self._frame_num = 0
        self.frame_surf = pygame.Surface(self.original_size)  # Reiniciamos la superficie
        self.active = True
        
    def set_size(self, size: tuple):
        self._video.set_size(*size)
        self.current_size = size
        self.frame_surf = pygame.Surface(self.current_size)  # Cambiamos el tamaño de la superficie
    ...
    def _update(self): 
        ...
        while self._video.get_pts() > self._frame_num * self.frame_delay:
            ...
            if frame is not None:
                ...
                self.frame_surf = img  # Asignamos el frame a la superficie de forma correcta
    ...
    def draw(self, surf: pygame.Surface, pos: tuple, force_draw: bool = True) -> bool:
        if self.active and (self._update() or force_draw):
            if self.frame_surf is not None:  # Nos aseguramos de que frame_surf no sea None
